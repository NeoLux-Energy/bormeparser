...
BORME_XML_URL = (
    "{protocol}://www.boe.es/datosabiertos/api/borme/sumario/{year}{month:02d}{day:02d}"
)
...
def get_nbo_from_xml(source):
    ...
    return tree.xpath("//response/data/sumario/diario")[0].attrib["nbo"]
...
def get_url_pdfs_provincia(date, provincia, secure=USE_HTTPS):
    ...
    urls = {}
    for item in tree.xpath("//response/data/sumario/item"):
...
def get_url_pdfs_seccion(date, seccion, secure=USE_HTTPS):
    ...
    for item in tree.xpath(
        '//response/data/sumario/diario/seccion[@codigo="{}"]/item'.format(seccion)
    ):
...
def get_url_seccion_c(date, format="xml", secure=USE_HTTPS):
    ...
    for item_emisor in tree.xpath('//response/data/sumario/diario/seccion[@codigo="C"]/apartado'):
