...
class BormeXML(object):
    ...
    def get_url_cve(self, cve):
        ...
        xpath = '//response/data/sumario/diario/seccion/emisor/item[@id="{}"]/url_pdf/text()'.format(
            cve
        )
    ...
    def get_provincias(self, seccion):
        xpath = '//response/data/sumario/diario/seccion[@codigo="{}"]/item/titulo/text()'.format(
            seccion
        )
    ...
    def _build_xpath(self, seccion=None, provincia=None):
        ...
        if seccion and provincia:
            ...
            xpath = '//response/data/sumario/diario/seccion[@codigo="{}"]/item'.format(
                seccion
            )
        elif provincia:
            xpath = '//sumario/diario/seccion/emisor/item/titulo[text()="{}"]'.format(
                provincia
            )
        else:
    ...
    def _get_url_borme_c(self, format="xml"):
        ...
        xpath_query = '//response/data/sumario/diario/seccion[@codigo="C"]/item'
        for item in self.xml.xpath(xpath_query):
            if format == "xml":
                url = url_base + item.xpath("url_xml")[0].text
            elif format in ("htm", "html"):
                url = url_base + item.xpath("url_html")[0].text
            elif format == "pdf":
                url = url_base + item.xpath("url_pdf")[0].text
            cve = item.get("id")
